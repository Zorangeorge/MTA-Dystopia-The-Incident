...
--[[		if animation then Animation:remove() end
		helpText:text( "Scroll down: melee weapons\nRight-click:  switch to ranged" )
		fadeInHelp()
...
addEventHandler("onClientResourceStart",getResourceRootElement(getThisResource()),
	function()
		...
		--bindKey ( "mouse_wheel_down", "down", "Change driveby weapon", "1" )
		bindKey ( "mouse_wheel_up", "down", "Previous driveby weapon", "-1" )
		...
		helpText = dxText:create("",0.91,0.80)
		helpText:scale(1)
...
addEventHandler("doSendDriveBySettings",localPlayer,
	function(newSettings)
		...
		-- settings.bike_passenger_allowed = {}
		-- if settings.passenger[1] then
			-- for i=1, #settings.passenger do
				-- settings.passengerallowed[settings.passenger[i]] = true
			-- end
		...
		-- settings.bike_driver_allowed = {}
		-- if settings.bike_driver[1] then
			-- for i=1, #settings.bike_driver do
				-- settings.bike_driver_allowed[settings.bike_driver[i]] = true
			-- end
		-- end
...
--[[local bike_models = { [581]=true,[509]=true,[481]=true,[462]=true,
--[521]=true,[463]=true,[510]=true,[522]=true,[461]=true,[448]=true,[468]=true,
[586]=true,[523]=true, [471]=true
}]]
...
function toggleDriveby()
	...
	--local is_bike = bike_models[vehicleID]
	...
	if equipedWeapon == 0 then
		...
		--[[if is_bike then -----------
		weaponsTableAllowed = bike_driver and settings.bike_driver_allowed or settings.bike_passenger_allowed -------------
		end------------]]
...
--[[			if animation then Animation:remove() end
		...
		--if shooting_veh_minigun then shooting_veh_minigungle() end
		setPedWeaponSlot( localPlayer, 0 )
		limitDrivebySpeed ( switchToWeapon )
	...
	--if switchTo == 4 and switchToWeapon ==32 and not shooting_veh_minigun then shooting_veh_minigungle() end
	--if switchTo ~= 4 or switchToWeapon ~=32 and shooting_veh_minigun then shooting_veh_minigungle() end
...
function switchDrivebyWeapon(key,progress)
	...
	for key,weaponID in ipairs(weaponsTable) do
		if weaponID == currentWeapon or not weaponsTableAllowed[currentWeapon] then
			...
			while i ~= key do
				theNextWeapon = weaponsTable[i]
				if theNextWeapon then
					local slot = getSlotFromWeapon ( theNextWeapon )
					local weapon = getPedWeapon ( localPlayer, slot )
					if ( weapon == theNextWeapon  ) then
...
local function onWeaponSwitchWhileDriveby (prevSlot, curSlot)
	...
	if prevSlot == 4 and getPedWeapon(source,4) == 32 then disableableVehMinigunViewpoint() 
	end
	
	if curSlot == 4 and getPedWeapon(source,4) == 32 then 
	enableVehMinigunViewpoint() 
	end
