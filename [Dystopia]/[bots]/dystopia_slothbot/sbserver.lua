...
function peddeath(ammo, attacker, weapon, bodypart)
	if (getElementData (source, "slothbot") == true) then -- if its a slothbot
		...
		local backCol = getElementData(source,"backSide")
		if isElement(backCol) then destroyElement(backCol) end
		...
		if (attacker) then
			if (getElementType(attacker) == "ped") and (getElementData (attacker, "slothbot") == true) then
				...
				setElementData(attacker,"status","waiting",true) --resets bot when it's target is dead
				setElementData (attacker, "target",nil,true)
			end
		end
	end	
end
...
--==============================
--{"I see you!","Somebody's there!","Who's there?","There's one!","Somebody's there!","There's one!",}
militaryAlertLines = {"Hostiles!","Kill him!","Engaging target!","Contact!","Go, go!","Flank him!","Enemy!","Movement!","We got hostiles!","Enemy activity!","Huh?!"}
badGuysAlertLines = {"Gotcha!","You there!","Get him!","I'll make you suffer!","You're dead!","Kill him!","Choose the easy way!","Aha!","I'm gon' get ya'!","Hah!","Huh?!"}
regularAlertLines = {"You there!","Get him!","Aha!","Hah!","Huh?!","You're dead!","Aha!","Hah!","Huh?!"}
raidersAlertLines = {"Aha!","You there!","Get him!","Hey, a little piggy!","Yeeehaw!","Meat!","Kill him!","Groaaa!","I'm gon' get ya'!","Come here, meatboy!","You're gonna get roasted!","I'd have you for dinner!","Hah!","Huh?!"}
...
function assigntarget ( player )
	if ( not wasEventCancelled() ) then
		...
		if rand1>0 then
			local faction = getElementData(source,"BotTeam")
			local alertstable
			local rand2 = math.random(500,2000)
			
			if getTeamName(faction) == "Establishment" or getTeamName(faction) == "CDF" then 
				alertstable = militaryAlertLines 
			elseif (getTeamName(faction) == "Bandits" or (getTeamName(faction) == "") and getElementData(source,"name")~= "Bear") then
				alertstable = badGuysAlertLines
			elseif getTeamName(faction) == "Raiders" then
				alertstable = raidersAlertLines
			else
							...
							if (pdistance < 1.2 ) then --if the ped is stuck
								...
								elseif decide <4 then -- jump
									...
									if (weap == 1) or (weap == 7) then
										setPedWeaponSlot(ped, 0 )
										triggerClientEvent ( "bot_Jump", ped )
										setTimer ( function(ped,weap) if isElement(ped) then setPedWeaponSlot(ped,weap) end end, 850, 1, ped, weap)
									else
										triggerClientEvent ( "bot_Jump", ped )
									end
									setTimer ( chase_move, 600, 1, ped, tx, ty, tz, px, py, pz)
...
function deploySpikestripLurker (ped)
        ...
        if (ped) then 
			...
			setTimer(function() destroyElement(spike); destroyElement(blow) end,60000,1)
	...
	if (isElement(source)) then
		if getElementType (source) == "ped" and (getElementData (source, "slothbot") == true) and (getElementData (source, "AllowFire") ~= false ) then
			...
			elseif type == "projectile" then
				triggerClientEvent (getRootElement(), "onProjectileThrow", source, source )
			elseif type == "barrel" then
				triggerClientEvent (getRootElement(), "onBarrelThrow", source, source )
				triggerClientEvent(root, "sync.message", source, source, 255, 0, 0, "#@$%!")
			elseif type == "spikestrip" then
				deploySpikestripLurker(source)
				triggerClientEvent(root, "sync.message", source, source, 255, 0, 0, "#@$%!")
			end
		end
	end
			...
			if (getElementData ( leader, "status" ) ~= "following" ) and (getElementData ( leader, "status" ) ~= "waiting" ) then
				if getElementType(leader) == "player" then
					if (getElementData ( ped, "BotTeam" ) == getPlayerTeam (leader)) then
						...
						triggerEvent ( "onBotFollow", ped, leader )
						triggerClientEvent(root, "sync.message", ped, ped, 125, 125, 125, "FOLLOWING")
					else 
						return false
					end
...
function SetBotWeapon (weapon)
	setTimer ( function() if isElement(source) then giveWeapon(source, tonumber(weapon), 999999, true ) end end, 300, 1)
...
function setBotFollow(ped, leader)
	if (isElement(ped)) and (isElement(leader)) then
		if (getElementData ( ped, "status" ) ~= "following" ) then
			...
			elseif getElementType(leader) == "ped" then
				if getElementData(ped,"BotTeam") == getElementData ( leader, "BotTeam" ) then
					if isElement(ped) then setElementData ( ped, "target", nil ) end
					setElementData ( ped, "leader", leader )
					assigncontroller(ped)
					triggerEvent ( "onBotFollow", ped, leader ) 
					triggerClientEvent(root, "sync.message", ped, ped, 125, 125, 125, "FOLLOWING")
...
function setBotGuard (theped, x, y, z, priority)
	if (isElement(theped)) then
		if (getElementData (theped, "slothbot") == true) then
			...
			assigncontroller(theped)
			setElementData ( guardcol, "type", "guardcol" )
			setElementData ( guardcol, "botassigned", theped )
...
--LOADS a follower INTO A CAR
function addFollowerToVehicle (vehicle, seat, jacked )
--outputDebugString("event triggered")
	if getElementData(source,"botBodyguard") then
	--outputDebugString("you have a follow")
		local follower = getElementData(source,"botBodyguard")
		local carid =  getElementModel( vehicle )
		local plx, ply, plz = getElementPosition ( source )
		local pedx, pedy, pedz = getElementPosition ( follower )
		local dist = getDistanceBetweenPoints2D ( plx, ply, pedx, pedy )
		
		--setElementData(follower,"slothbot",nil,true)
		--setElementData(follower,"type",nil,true)
		--outputDebugString("follow nilized")	
		--outputDebugString(getElementData(follower,"name"))	
		if ( dist < 6 ) then
		outputDebugString("he is in range")
			if (getVehicleOccupant ( vehicle, 1 ) == false) then
			outputDebugString("warped 1")
				warpPedIntoVehicle ( follower, vehicle, 1 )
				--setPedDoingGangDriveby( follower , true)
			elseif (getVehicleOccupant ( vehicle, 2 ) == false) then
				warpPedIntoVehicle ( follower, vehicle, 2 )
				--setPedDoingGangDriveby( follower , true)
				outputDebugString("warped 2")
			elseif (getVehicleOccupant ( vehicle, 3 ) == false) then
				warpPedIntoVehicle ( follower, vehicle, 3 )
				--setPedDoingGangDriveby( follower , true)
				outputDebugString("warped 3")
			end
		end
...
addEventHandler("onPlayerVehicleEnter",root,addFollowerToVehicle)

function removeFollowerFromVehicle (vehicle, seat, jacked )
--outputDebugString("event triggered")
	if getElementData(source,"botBodyguard") then
	outputDebugString("you have a follow")
		local follower = getElementData(source,"botBodyguard")
		--setPedDoingGangDriveby( follower , false)
		removePedFromVehicle(follower)
		--outputDebugString("he got removed")
		
	end

end
addEventHandler("onPlayerVehicleExit",root,removeFollowerFromVehicle)