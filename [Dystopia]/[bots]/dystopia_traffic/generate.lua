			...
			for sy = y-2,y+2 do 
				for sx = x-2,x+2 do
					...
					if sx == x and sy == y then
						if not square then square = createPopulationSquare(sx,sy,dim,"despawn")
						end
...
refugeeSkin = { 10,15,77,78,79, 32, 44, 58, 62, 95, 131, 132, 134, 137, 151, 157, 158, 159, 160 }
...
function explodeKamikaboom (vehicle)
if getElementModel(source) == 545 and getElementData(source,"setup") == "Kamikaboom" then blowVehicle(source) end
end
addEventHandler("onVehicleDamage", root,explodeKamikaboom)
PANIC_DIST = 100
PANIC_TIME = 60
PANIC_SPEED = 100
STOP_VEH = 0
		...
		if isPedDrivingVehicle(ped) then
			...
			local vehicle = getPedOccupiedVehicle(ped)
					...
					--iprint(vehicle)
						...
						elseif setup == "Raider Scout" or setup == "Bandit Scout" or setup == "Bandit Pickup" then
							call(npc_hlc,"setNPCDriveSpeed",ped,0)
							outputDebugString("TRAFFIC: Scout deployed!")
							
							local shooterB = getElementData(vehicle,"shooterB")
...
Las_Venturas_vehicle_setups = {"Raider Pickup","Raider Scout","Bandit Scout","Siegetruck","Roadwarrior","War Rig","Raider Lurker"}
San_Fierro_vehicle_setups = {"Bandit Pickup","Raider Lurker","Bandit Scout","Raider Pickup","Siegetruck","Raider Scout","Kamikaboom"}
Red_County_vehicle_setups = {"Siegetruck"}
Whetstone_vehicle_setups = {"Bandit Pickup","Raider Lurker","Bandit Scout","Raider Pickup","Siegetruck","Raider Scout","Kamikaboom"}
Flint_County_vehicle_setups = {"Bandit Pickup","Bandit Scout"}
--[[{
"Raider Pickup",--
"Bandit Pickup",--
"R01 Van",--
...
outputDebugString("vehicle spawned in: "..zone)
local pedteam
local pedskin
local pedWeapons
local setup
	...
	elseif zone == "Las Venturas" then setup = table.random(Las_Venturas_vehicle_setups)
	elseif zone == "San Fierro" then setup = table.random(San_Fierro_vehicle_setups)
	elseif zone == "Red County" then setup = table.random(Red_County_vehicle_setups)
	elseif zone == "Whetstone" then setup = table.random(Whetstone_vehicle_setups)
	elseif zone == "Flint County" then setup = table.random(Flint_County_vehicle_setups)
...
local shooter3x,shooter3y,shooter3z
local shooter4x,shooter4y,shooter4z
local shooterBx,shooterBy,shooterBz
local shooterTx,shooterTy,shooterTz
local maxpass = getVehicleMaxPassengers(car)
	...
	if setup == "Raider Pickup" then 
		...
		shooter1x,shooter1y,shooter1z = 0.6, -1, 1.3
		shooter2x,shooter2y,shooter2z = -0.6, -1, 1.3
		victim1x, victim1y, victim1z = 0, -1.7, 1
		pedteam = raiderteam
		pedskin = raiderSkin
		pedWeapons = raiderGunnerWeapons
	elseif setup == "Raider Scout" then 
		setElementModel(car,424) --bf inj
		setVehicleColor(car,raiderr,raiderg,raiderb,raiderr,raiderg,raiderb)
		shooter1x, shooter1y, shooter1z = 0.4, -0.2, 1
		shooterBx, shooterBy, shooterBz = -0.5, -1, 1
		pedteam = raiderteam
		pedskin = raiderSkin
		pedWeapons = raiderGunnerWeapons
	elseif setup == "Bandit Scout" then
		...
		shooterBx, shooterBy, shooterBz = 0.5, -1, 1
		shooter1x,shooter1y,shooter1z = 0.4, -0.2, 1
		shooter2x,shooter2y,shooter2z = -0.5, -1, 1
		pedteam = banditteam
		pedskin = banditSkin
		pedWeapons = {25}
	elseif setup == "Raider Lurker" then
		setElementModel(car,545) --hustler
		position_and_attach_objects(car)
		setVehicleDoorState(car,0,4)
		setVehicleDoorState(car,1,4)
		setVehicleDoorState(car,2,4)
		setVehicleDoorState(car,3,4)
		setVehicleColor(car,raiderr,raiderg,raiderb,raiderr,raiderg,raiderb)
		shooterTx, shooterTy, shooterTz = 0, -1.8, 0.7
		victim1x, victim1y, victim1z = 0, -0.4, 1.8
		pedteam = raiderteam
		pedskin = raiderSkin
		pedWeapons = raiderGunnerWeapons
	...
	elseif setup == "Bandit Pickup" then
		...
		setVehicleDoorState(car,3,4)
		setVehicleColor(car,banditr,banditg,banditb,banditr,banditg,banditb)
		shooter1x,shooter1y,shooter1z = 0.6, -1.3, 0.9
		shooterTx,shooterTy,shooterTz = 0, -2.2, 0.7
		shooterBx,shooterBy,shooterBz = -0.6, -1.3, 0.9
		pedteam = banditteam
		pedskin = banditSkin
	...
	elseif setup == "CDF Pickup" then --reinforced bobcat
		...
		shooter1x,shooter1y,shooter1z = 0.6, -1.3, 0.9
		shooter2x,shooter2y,shooter2z = -0.6, -1.3, 0.9
		shooterTx,shooterTy,shooterTz = 0, -2.2, 0.7
		pedteam = cdfteam
		pedskin = cdfSkin
		pedWeapons = cdfGunnerWeapons
	elseif setup == "Kamikaboom" then --kamikaze hustler
		setElementModel(car,545)
		position_and_attach_objects(car)
		setVehicleColor(car,raiderr,raiderg,raiderb,raiderr,raiderg,raiderb)
		...
		pedskin = raiderSkin
		pedWeapons = raiderGunnerWeapons
	elseif setup == "Siegetruck" then --raider packer
		setElementModel(car,443)
		--exports["extra_health"]:setElementExtraHealth (car,1000)
		position_and_attach_objects(car)
		setVehicleColor(car,raiderr,raiderg,raiderb,raiderr,raiderg,raiderb)
		shooter1x,shooter1y,shooter1z = 0, 5, 1.8
		shooter2x,shooter2y,shooter2z = 1.8, 3, 3.5
		shooter3x,shooter3y,shooter3z = -1.8, 3, 3.5
		shooterTx,shooterTy,shooterTz = 0, -6.7, 0.4
		pedteam = raiderteam
		pedskin = raiderSkin
		pedWeapons = raiderGunnerWeapons
	elseif setup == "Roadwarrior" then --raider packer
		setElementModel(car,549)
		position_and_attach_objects(car)
		setVehicleColor(car,raiderr,raiderg,raiderb,230,230,230)
		shooterBx, shooterBy, shooterBz = 0, 0, 1.7
		shooterTx, shooterTy, shooterTz = 0, -1.9, 1.2
		pedteam = raiderteam
		pedskin = raiderSkin
		pedWeapons = raiderGunnerWeapons
	elseif setup == "War Rig" then --raider 
		setElementModel(car,515)
		--exports["extra_health"]:setElementExtraHealth (car,1000)
		position_and_attach_objects(car)
		setVehicleColor(car,raiderr,raiderg,raiderb,raiderr,raiderg,raiderb)
		shooter1x,shooter1y,shooter1z = 0, -0.5, 2.6
		shooterTx,shooterTy,shooterTz = 0, -2, 0.5
		pedteam = raiderteam
		pedskin = raiderSkin
		pedWeapons = raiderGunnerWeapons
	elseif setup == "Peacekeeper Van" then --estab flat van
		setElementModel(car,552)
		position_and_attach_objects(car)
		setVehicleColor(car,47,61,90,158,164,171,47,61,90)
		shooter1x,shooter1y,shooter1z = 0.3, -2.6, 1
		shooter2x,shooter2y,shooter2z = 0, 0.9, 2.2
		victim1x, victim1y, victim1z = 0, -1, 1
		pedteam = estabteam
		pedskin = peaceSkin
		pedWeapons = cdfGunnerWeapons
	else
	...
	if 	shooter3x then
		local shooter3 = exports["dystopia_slothbot"]:spawnBot (x,y,z,r, table.random(pedskin), int, int, pedteam,table.random(pedWeapons), "guarding", nil)
		attachElements ( shooter3, car, shooter3x,shooter3y,shooter3z)
		setElementData(shooter3, "name", "Gunner")
		--setElementData(shooter3, "raider", true)
		setElementData(shooter3, "BotTeam", pedteam)
		setElementDimension(shooter3,dim)
		element_timers[shooter3] = {}
		addEventHandler("onElementDestroy",shooter3,removePedFromListOnDestroy,false)
		addEventHandler("onPedWasted",shooter3,removeDeadPed,false)
		population.peds[shooter3] = true
		setElementData(car,"shooter3",shooter3)

		shooter3x = false
		--local allPlayers = getElementsByType("player")

		--for k,v in ipairs(allPlayers) do
		--	triggerClientEvent(root,"pedCrouch",root,shooter3,true) ------zzz
		--end
	end

	if 	shooter4x then
		local shooter4 = exports["dystopia_slothbot"]:spawnBot (x,y,z,r, table.random(pedskin), int, int, pedteam, table.random(pedWeapons), "guarding", nil)
		attachElements ( shooter4, car, shooter4x,shooter4y,shooter4z)
		setElementData(shooter4, "name", "Gunner")
		--setElementData(shooter4, "raider", true)
		setElementData(shooter4, "BotTeam", pedteam)
		setElementDimension(shooter4,dim)
		element_timers[shooter4] = {}
		addEventHandler("onElementDestroy",shooter4,removePedFromListOnDestroy,false)
		addEventHandler("onPedWasted",shooter4,removeDeadPed,false)
		population.peds[shooter4] = true
		setElementData(car,"shooter4",shooter4)
	...
	if 	shooterBx then
		local shooterB = exports["dystopia_slothbot"]:spawnBot (x,y,z,r, table.random(pedskin), int, int, pedteam, table.random(pedWeapons), "guarding", nil)
...
--[[		local bottimer = setTimer(function ()
		...
		setElementData(car,"shooterB",shooterB,true)
	...
	if 	shooterTx then
		local shooterT = exports["dystopia_slothbot"]:spawnBot (x,y,z,r, table.random(pedskin), int, int, pedteam, 24, "guarding", nil)
		
		--exports["extra_health"]:setElementExtraHealth (shooterT,500)
		
		outputDebugString("shooterT created")
		--iprint(getPedWeapon(shooterT))
		attachElements ( shooterT, car, shooterTx,shooterTy,shooterTz)
		setElementData(shooterT,"name","Thrower")
		if setup == "Siegetruck" or setup == "Roadwarrior" then
		setElementData(shooterT,"specialty","barrel",true)
		elseif setup == "Raider Lurker" or setup == "CDF Pickup" then
		setElementData(shooterT,"specialty","spikestrip",true)
		end
		

		--warpPedIntoVehicle(shooterT,car,1)
		--[[local bottimer = setTimer(function ()

			if shooterT and veh and getElementData(shooterT,"target") and getElementType(getElementData(shooterT,"target"))
			=="player" and getPedOccupiedVehicle(getElementData(shooterT,"target")) then

				local veh = car
				local x,y,z = getElementPosition(shooterT)
				local rx,ry,rz = getElementRotation(shooterT)
				setPedAnimation(shooterT,"grenade","weapon_throw",-1,false,false,false,false)
				triggerClientEvent(root,"pedThrowsMolotov",shooterT,...)
			end
		end,10000,0)]]

		setElementData(shooterT, "BotTeam",pedteam)
		--setElementData(shooterB, "stucktimer",bottimer)
		setElementDimension(shooterT,dim)
		element_timers[shooterT] = {}
		addEventHandler("onElementDestroy",shooterT,removePedFromListOnDestroy,false)
		addEventHandler("onPedWasted",shooterT,removeDeadPed,false)
		population.peds[shooterT] = true
		setElementData(car,"shooterT",shooterT)
		--iprint("attached")--
		shooterTx = false
	...
	if 	victim1x then	
		...
		if setup == "Raider Lurker" or setup == "Raider Pickup" then
					local victim1 = createPed(table.random(meatSkin), x,y,z)
					setElementData(victim1, "BotTeam", pedteam)
					attachElements ( victim1, car, victim1x,victim1y,victim1z)
					setElementData(victim1, "name", "Meat")
					setElementData (victim1,"raidermeat",true)
					if setup == "Raider Lurker" then
					setPedAnimation(victim1,"WUZI", "CS_Dead_Guy",-1, true, false, true, false)
					else
					setPedAnimation(victim1,"SWEET","Sweet_injuredloop",-1, true, false, true, false)
					end
					setElementRotation(victim1,getElementRotation(car))
					setElementDimension(victim1,dim)
					element_timers[victim1] = {}
					addEventHandler("onElementDestroy",victim1,removePedFromListOnDestroy,false)
					addEventHandler("onPedWasted",victim1,removeDeadPed,false)
					population.peds[victim1] = true
					setElementData(car,"victim1",victim1)
					
		end
		
		if setup == "Peacekeeper Van" then
					local victim1 = createPed(table.random(refugeeSkin), x,y,z)
					setElementData(victim1, "BotTeam", pedteam)
					attachElements ( victim1, car, victim1x,victim1y,victim1z)
					setElementData(victim1, "name", "Captive")
					setElementData (victim1,"prisoner",true)
					setPedAnimation(victim1,"SWEET","Sweet_injuredloop",-1, true, false, true, false)
					setElementRotation(victim1,getElementRotation(car))
					setElementDimension(victim1,dim)
					element_timers[victim1] = {}
