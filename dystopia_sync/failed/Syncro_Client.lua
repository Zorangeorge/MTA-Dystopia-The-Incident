...
WeaponInaccuracy[38]=400 -- Minigun
...
function SYNCRO_INIT()
	if source == getThisResource() then
		outputDebugString("Starting pl-Syncro");
	end
	bIsSpawned = nil;
...
function syncro (attacker,weap,bodyp,loss)
	...
	if attacker then
		if getElementType(attacker) =="vehicle" then 
			if hploss ~= 0 then
			triggerEvent("sync.message", root, source, 255, 0, 0, hploss)
			end
			return 
		elseif getElementType(attacker) =="ped" and getDistanceBetweenPoints3D(ax,ay,az,tx,ty,tz)<0.7 then
...
function onClientPlayerWeaponFireFunc_CallBack(source, weapon, ammo, ammoInClip, hitX, hitY, hitZ, hitElement )
	if getElementHealth(source)<1 or iFrameCount < 1 then -- old function! new one: isPedDead( ped )
		return; 
	end
	iFrameCount = 0;
	local bodypart = "";
...
function SYNCRO_RENDERFRAME()
	...
	if (getCameraTarget() ~= getLocalPlayer()) 
		...
		or doesPedHaveJetPack(getLocalPlayer())
	then
		return;
	end
...
function SYNCRO_PEDWEAPONFIRE( weapon, ammo, ammoInClip, hitX, hitY, hitZ, hitElement )
	...
	if target == getLocalPlayer() then
		triggerServerEvent("S_PEDSHOT",root,target,ped,weapon,hitX,hitY,hitZ);
	else
		triggerServerEvent("S_PEDSHOTPED",root,target,ped,weapon,hitX,hitY,hitZ);
	end
end
...
function SYNC_STOMP()
	...
	if (playerIsAiming and  isControlEnabled ( "enter_exit" )) then
		...
		elseif (weapon ~= -1 and (weapon > 16 and weapon ~= 37)) then
			setPedAnimation(localPlayer, "FIGHT_D","FightD_1",900,false,true,true,false)
			triggerServerEvent("SyncWeapStomp",localPlayer)
