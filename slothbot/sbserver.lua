...
function SetBotWeapon (weapon)
    if source and getElementType(source) == "ped" then
	    ...
	    setTimer (function (source)
			if isElement( source ) then
				giveWeapon(source, tonumber(weapon), 9999, true)
			end
		end , 850, 1)
...
--checks the function commands to see if all neccesary parts are filled, sets defaults if not or returns false.
	...
	if (slothbot ~= false) then
		...
		setTimerElementData ( setElementData, 200, 1, slothbot, "slothbot", true ) -- makes it a bot
		setTimerElementData ( setElementData, 200, 1, slothbot, "AllowFire", true ) -- makes it able to shoot when it wants
		...
		setTimer (function (slothbot)
				if isElement( slothbot ) then
					giveWeapon(slothbot, tonumber(weapon), 9999, true)
				end
			end , 850, 1)
		...
		setTimer (function (slothbot)
				if isElement( slothbot ) then
					setElementInterior(slothbot, tonumber(dimension))
				end
			end , 100, 1)
		
		--sets the mode
		if mode == "waiting" then
			setTimerElementData ( setElementData, 600, 1, slothbot, "status", "waiting")
		elseif mode == "following" then
			setTimerElementData ( setElementData, 400, 1, slothbot, "leader", modesubject )
			setTimerElementData ( setElementData, 600, 1, slothbot, "status", "following")
		elseif mode == "chasing" then
			setTimerElementData ( setElementData, 400, 1, slothbot, "target", modesubject )
			setTimerElementData ( setElementData, 600, 1, slothbot, "status", "chasing")
		elseif mode == "guarding" then
			setTimer ( setBotGuard, 400, 1, slothbot, x, y, z)
		else
			setTimerElementData ( setElementData, 600, 1, slothbot, "status", "hunting")
		end
...
function setBotWeapon(ped, weapon)
	if (isElement(ped)) and getWeaponNameFromID(tonumber(weapon)) ~= false then		
		if (getElementData (ped, "slothbot") == true) then
			...
			setTimer (function (ped)
				if isElement( ped ) then
					giveWeapon(ped,  tonumber(weapon), 99999, true )
				end
			end , 100, 1)
...
function setTimerElementData(thefunc, timeInterval, timesToExecute, source, type, data)
	setTimer( function ( )
		if isElement( source ) then
			setElementData( source, type, data )
		end
	end, timeInterval, timesToExecute)
end