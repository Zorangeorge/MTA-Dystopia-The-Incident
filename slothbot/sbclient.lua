...
function followshoot(ped)
	...
	if (getPedWeapon(ped) < 19)  and (getPedWeapon(ped) ~= 9) then -- if its a melee weapon that isnt a chainsaw
		if (distance < 2 ) then
			triggerServerEvent ( "pedShootTrigger", ped, "melee" )
			setTimerElementData ( setElementData, 2100, 1, ped, "follow_shooting", false )
		end
	elseif (getPedWeapon(ped) == 9) then -- if it is a chainsaw (watch out, the peds are deadly with these)
		if (distance < 2 ) then
			local length = 1500
			triggerServerEvent ( "pedShootTrigger", ped, "chainsawA", length )
			setTimerElementData ( setElementData, length+100, 1, ped, "follow_shooting", false )
		else
			local length = math.random( 2100, 5500 )
			triggerServerEvent ( "pedShootTrigger", ped, "chainsawB", length )
			setTimerElementData ( setElementData, length+100, 1, ped, "follow_shooting", false )
	...
	elseif (getPedWeapon(ped) == 34) then
		if (distance < 75 ) then
			triggerServerEvent ( "pedShootTrigger", ped, "sniper")
			setTimerElementData ( setElementData, 1000, 1, ped, "follow_shooting", false )
		end
	elseif (getPedWeapon(ped) == 37) then
		if (distance < 15 ) then
			local length = math.random( 2000, 5500 )
...
function guardshoot(ped)
	if (getElementData( ped, "forcedmoving" ) ~= true) then
		...
		if (getPedWeapon(ped) < 19)  and (getPedWeapon(ped) ~= 9) then -- if its a melee weapon that isnt a chainsaw
			if (distance < 2 ) then
				triggerServerEvent ( "pedShootTrigger", ped, "melee" )
				setTimerElementData ( setElementData, 2100, 1, ped, "guard_shooting", false )
			end
		elseif (getPedWeapon(ped) == 9) then -- if it is a chainsaw (watch out, the peds are deadly with these)
			if (distance < 2 ) then
				local length = 1500
				triggerServerEvent ( "pedShootTrigger", ped, "chainsawA", length )
				setTimerElementData ( setElementData, length+100, 1, ped, "guard_shooting", false )
			else
				local length = math.random( 2100, 5500 )
				triggerServerEvent ( "pedShootTrigger", ped, "chainsawB", length )
				setTimerElementData ( setElementData, length+100, 1, ped, "guard_shooting", false )
		...
		elseif (getPedWeapon(ped) == 34) then
			if (distance < 75 ) then
				triggerServerEvent ( "pedShootTrigger", ped, "sniper")
				setTimerElementData ( setElementData, 1000, 1, ped, "guard_shooting", false )
			end
		elseif (getPedWeapon(ped) == 37) then
			if (distance < 15 ) then
				local length = math.random( 2000, 5500 )
